...
- **The merge between `master` and `beta` stopped on a conflict** (after a release, or while building the stable release PR): the release itself is out. Merge the branches by hand, keeping the target branch's `.plg`, and push; the next run carries on from there.
