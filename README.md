...
- On scheduled runs of mover
    - Only actually move file(s) if the the cache drive is getting full (selectable thresholds).
...
- Manually executed runs of mover ("Move Now" button) or via command line ("mover start") can either follow schedule rules or always move all files.
